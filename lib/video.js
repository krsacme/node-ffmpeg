...
module.exports = function (filePath, settings, infoConfiguration, infoFile) {
	...
	this.fnExtractFrameToJPG = function (/* destinationFolder, settings, callback */) {
		...
		if (newDimension.aspect != null) {
			this.addFilterComplex('scale=min(iw\\,iw*sar):ih, pad=max(iw\\,ih*(' + newDimension.aspect.x + '/' + newDimension.aspect.y + ')):ow/(' + newDimension.aspect.x + '/' + newDimension.aspect.y + '):(ow-iw)/2:(oh-ih)/2' + (settings.padding_color != null ? ':' + settings.padding_color : ''));
			this.addCommand('-aspect', newDimension.aspect.string);
		}
